...
If you are a part of [Google organization](https://github.com/orgs/google/people), please see skip to [Kaniko Release Process - Google Contributors](https://github.com/cniry/kaniko/blob/master/RELEASE.md#kaniko-release-process-google-contributors)

Non-Google Contributors or users, please follow the following steps:
1. Follow the setup instruction to fork kaniko repository [here](https://github.com/cniry/kaniko/blob/master/DEVELOPMENT.md#getting-started)
...
1. Create a release PR and update Changelog.
    ...
    a. Create a new branch and bump the kaniko version in [Makefile](https://github.com/cniry/kaniko/blob/master/Makefile#L16)
    ...
    b. Run the [script](https://github.com/cniry/kaniko/blob/master/hack/release.sh) to create release notes.
    ```
    ./hack/release.sh
    Collecting pull request that were merged since the last release: v1.0.0 (2020-08-18 02:53:46 +0000 UTC)
    * change repo string to just string [#1417](https://github.com/cniry/kaniko/pull/1417)
    * Improve --use-new-run help text, update README with missing flags [#1405](https://github.com/cniry/kaniko/pull/1405)
    ...
    - Alexander Sharov
    ```
    Copy the release notes and update the [CHANGELOG.md](https://github.com/cniry/kaniko/blob/master/CHANGELOG.md) at the root of the repository. 

    c. Create a pull request like [this](https://github.com/cniry/kaniko/pull/1388) and get it approved from Kaniko maintainers.
...
2. Once the PR is approved and merged, create a release tag with name `vX.Y.Z` where
    ...
    ```
3.  Pushing a tag to remote with above naming convention will trigger the Github workflow action defined [here](https://github.com/cniry/kaniko/blob/main/.github/workflows/images.yaml) It takes 20-30 mins for the job to finish and push images to [`kaniko-project`](https://pantheon.corp.google.com/gcr/images/kaniko-project?orgonly=true&project=kaniko-project&supportedpurview=organizationId)
```
...
4. Finally, once the images are published, create a release for the newly created [tag](https://github.com/cniry/kaniko/tags) and publish it. 
